				...
				<c:when test="${f:length(list) > 0}">
					...
					<c:forEach items="${list }" var="row" varStatus="status">
						<tr style="text-align: center;">
							<td class="checkbox">
								<ul class="select_subject">
									<input type="checkbox" checked="checked" class="input_check"
										id="${totpay }" name="check_li" value="${status.current.wish_p_id }"
										onclick="func1(this)" />
									<input type="hidden" id="sub_payment"
										value="${row.p_price * row.wish_number }">
								</ul>
							</td>
							<td>${row.wish_p_id }</td>
							<td class="title" style="text-align: center;"><a
								href="#this" name="title">${row.p_name }</a> <input type="hidden"
								id="id" value="${row.wish_p_id }"> <input type="hidden"
								id="user_id" value="${userid }"></td>

							<td>
							<input type="number" id="number${sel_number }" name="number" max="100"
								min="1" value="${row.wish_number }" class="wdp_38p" >
							...
							<td id="price_sel">${row.p_price}</td>
							<td id="sub_pay${sub_pay }">${row.p_price * row.wish_number}</td>
							<td>
								<a href="#this" class="btn" id="go_empty" name="empty">비우기</a>
								<input type="hidden" id="empty_no" value="${status.current.wish_p_id }">
							</td>
						</tr>
						<c:set var="tot" value="${tot + row.p_price * row.wish_number }" />
