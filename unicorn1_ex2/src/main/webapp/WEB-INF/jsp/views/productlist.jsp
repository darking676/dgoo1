                ...
                <c:forEach items="${list }" var="row" varStatus="status">
                    <tr style="text-align: center;">
                        ...
                        <td class="title" style="text-align: center;">
                        	<a href="#this" name="title" value="${status.current.p_name }">${row.p_name }</a>
                        	<input type="hidden" id="id" value="${row.p_id }">
                        	<input type="hidden" id="user_id" value="${userid}">	                        	
                        	<input type="hidden" id="stock" value="${status.current.p_stock }">	                        	
                        	<input type="number" id="number" name="wish_number" max="100" min="1" value="1" class="wdp_38p" onchange="config_stock(this);">	
                        </td>
                        <td>${row.p_price }</td>
                        <td id="stock">${row.p_stock }</td>
                        <td>
...
<script type="text/javascript">
        ...
        function fn_go_cart(obj){
        	var url = 'ProductDetail.do?p_id='+obj.parent().find("#p_id").val();
        	window.open(url, '', 'width=500, height=450, top=200, left=300');
            ...
            if(confirm("장바구니에 담겠습니까?") == true){
            	if($("#user_id").val() == ""){ 
		            ...
		            comSubmit.setUrl("<c:url value='/views/WishList.do' />");
		            comSubmit.addParam("wish_number", obj.parent().parent().find("#number").val());
		            comSubmit.addParam("wish_p_id", obj.parent().find("#p_id").val());
		           	comSubmit.submit();
            	}
           	}else{
            	return;
